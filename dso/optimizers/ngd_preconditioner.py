...
from dso.utils.factors import ComputeI, ComputeG
from dso.utils.comm import TorchBackend
...
class NGDOptimizer(Optimizer):
    def __init__(
        ...
        *,
        ...
        defaults = dict(damping=init_damping)
        ...
        self.damping_decay_step = damping_decay_step
        self.freq = freq
        ...
        self.acc_stats = True
        self.steps = 0
        # KID is lower approx error and KIS is faster to compute
        # randk = False is KID
        if self.adaptive is not None:
            self.randk = not (
                (self.steps < self.iters_per_epoch * self.adaptive[0])
                or (
                    self.steps > self.iters_per_epoch * self.adaptive[1]
                    and self.steps <= self.iters_per_epoch * self.adaptive[2]
                )
            )
        else:
            self.randk = False
        self._prepare_model(skip_layers)

    def _save_input(self, m, input):
        if torch.is_grad_enabled() and self.steps % self.freq == 0:
            data = input[0].data.to(torch.float32)
            if len(data.size()) == 3:
                I = self.IHandler(data[:, 0, :], m)
                for i in range(1, data.size(1)):
                    I += self.IHandler(data[:, i, :], m)
                self.m_I[m] = I / data.size(1)
            elif len(data.size()) == 2:
                self.m_I[m] = self.IHandler(data, m)
            else:
                raise RuntimeError(f'Got tensor with shape {data.size()}.')

    def _save_grad_output(self, m, grad_input, grad_output):
        if self.acc_stats and self.steps % self.freq == 0:
            data = grad_output[0].data.to(torch.float32)
            if len(data.size()) == 3:
                G, _ = self.GHandler(data[:, 0, :], m)
                for i in range(1, data.size(1)):
                    G_i, _ = self.GHandler(data[:, i, :], m)
                    G += G_i
                G = G / data.size(1)
            elif len(data.size()) == 2:
                G, _ = self.GHandler(data, m)
            else:
                raise RuntimeError(f'Got tensor with shape {data.size()}.')
    ...
    def _prepare_model(self, skip_layers: list[str] | None = None):
        count = 0
        ...
        for name, m in self.model.named_modules():
            ...
            if (
                not any(skip in name for skip in skip_layers)
                and classname in self.known_modules
            ):
                ...
                if self.backend.rank() == 0:
                    print(f'Registering {name} (#{count}): {classname}')
            else:
                if self.backend.rank() == 0:
                    print(f'Skipping {name}: {classname}')
    ...
    def allgather_factors(self):
        ...
        if not self.randk:
            for m in self.modules:
                ...
                self.m_RDR[m] = [
                    mat[i * Rsize : (i + 1) * Rsize, :] for i in range(RDR_len)
                ]
                offset += Rsize
    ...
    def _interpolative_decomposition(self, K, r):
        ...
        assert not torch.isnan(R11).any()
    ...
    def interpolative_decomposition(self, m):
        ...
        Dinv = torch.inverse(
             residual + self.damping * torch.eye(residual.size(0)).to(residual.device)
        )
    ...
    def compute_projection(self):
        for m in self.modules:
            ...
            self.m_K[m] = (
                ...
                - RDR
                @ K
                @ torch.inverse(torch.eye(K.shape[0]).to(K.device) + RDR @ K)
                @ RDR
            )
    ...
    def _precondition(self, m):
        ...
        bias_update = (
            bias_update.view_as(bias).contiguous() if bias_update is not None else None
        )
    ...
    def _broadcast_preconditioned_gradients(self, m, updates):
        return [
            self.backend.broadcast(tensor, src=self.assigned_worker[m])
            for tensor in updates[m]
    ...
    def step(self, closure=None):
        if self.steps % self.freq == 0:
            ...
            # if self.steps >= self.damping_decay_step:
            #     alpha = torch.Tensor(self.init_damping / self.target_damping)
            #     alpha = 2 * torch.log10(alpha) / self.damping_decay_step
